...
public class NutritionControllerTest {
    ...
    public void testCreateSupplier() throws Exception {
    	...
    	supplier mockSupplier = new supplier(1L, "Supplier1", "Item1", "IC1", "10.00", "20.00", "10.00");

        // mock the repository save method to return the sample supplier object
       ...
       supplier result = nutritionController.create(mockSupplier);
        ...
        // perform a POST request to the /supplier endpoint with the sample supplier object as request body
		/*
		 * mockMvc.perform(post("/supplier") .contentType(MediaType.APPLICATION_JSON)
		 * .content(asJsonString(testSupplier))) .andExpect(status().isOk())
		 * .andExpect(jsonPath("$.id", is(1L))) .andExpect(jsonPath("$.supplier_name",
		 * is("Supplier1"))) .andExpect(jsonPath("$.item", is("Item1")))
		 * .andExpect(jsonPath("$.item_code", is("Code1")))
		 * .andExpect(jsonPath("$.manufacting_cost", is("10.0")))
		 * .andExpect(jsonPath("$.selling_cost", is("20.0")))
		 * .andExpect(jsonPath("$.profit_gained", is("10.0")));
		 */

        // verify that the repository save method was called once with the sample supplier object as argument
